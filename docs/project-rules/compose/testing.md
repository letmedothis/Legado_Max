...
- **注入接口只收"有决策"的成员**：格式文案、纯路径拼接等无决策方法不进接口——塞进去了，Fake 就得实现空壳（`StorageDataProvider` 现有 29 个方法多数如此，属反例，随迭代收敛）。接口越瘦，Fake 成本越低、越不容易造假。
- **能直接用真依赖就优先测真的**：`FileManageViewModelTest` 直接传真实 `rootDir` 测文件排序/删除，不抽接口也不用 Fake，是正面例子——别为测试而造接口。确需 Fake 时，重实现业务语义（如 `FakeReadRecordDao` 的内存实现）要在测试注释里标注与真身（Default 实现 / DAO SQL）的同步责任，并规划用 `room-testing` 内存库补一层真 DAO——Fake 与真身"错得一致"时测试等于没测。
...
- **轮询/无限循环协程（如下载页 `while(true) + delay(500)`）禁止 `advanceUntilIdle`**——虚拟时间下会无限推进导致 `runTest` 超时；改用 `runCurrent()` / `advanceTimeBy(轮询周期)` 精确驱动。与上一条是两条线：那条禁的是"断言中间帧"，这条禁的是"驱动方式用错工具没收敛"。
