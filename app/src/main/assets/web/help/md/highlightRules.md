...
| `bgSpacingH` | 0 | 左右间距（em，随字号缩放）：**正数**把背景向外撑大、离文字更远，**负数**向内收（范围 −0.50 ~ +0.50em） |
...
| 强制 | 四角尺寸（分割比例 × 字号），并把左右邻字各向外推开"外扩量 + 一个正文字距 − 邻字原有空隙" | 四角尺寸（分割比例 × 字号） |

> **强制模式下邻字会被推开**：推开的距离作为**真实排版宽度**参与断行与两端对齐，所以剩下的文字仍然整齐（不会溢出右边距，也不会参差不齐）；单侧最多让出 1em，避免极端分割比例把整行挤爆。"一个正文字距"取自阅读设置里的「字间距」，不需要额外配置。

> **四角尺寸怎么算**：按"分割比例 × 字号"等比缩放后绘制（水平方向再乘图片宽高比），**与图片自身分辨率无关**——换一张分辨率差 10 倍的图，效果完全一样；四角始终保持原有长宽比，不会被单向压扁。想让边框更厚就调大分割比例，想更薄就调小。四角与边条全部画在匹配文本范围内，加上自动外扩与间距后才可能超出，超出部分最多为"四角尺寸 + 间距"。
